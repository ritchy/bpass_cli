...
//import 'package:password_manager/service/crypto_utils.dart';
import 'dart:io';
import 'dart:math';
...
final log = Logger('EncryptionHandler');
...
// here is how you generate key pair
  final pair = generateRSAkeyPair(generateSecureRandom());
  final publicKey = pair.publicKey; // to get public
  final privateKey = pair.privateKey;
  //savePemFiles(publicKey, privateKey);
  String outcome = await testPemFiles("howdy Robby", publicKey, privateKey);

  log.info("generateExchangePems(): outcome: $outcome");
  return pair;
}

Future<void> generateRSAPemFiles(
    String publicFilePath, String privateFilePath) async {
  AsymmetricKeyPair pair = await generateKeyPair();
  // Cast the generated key pair into the RSA key types
  final publicKey = pair.publicKey as RSAPublicKey;
  final privateKey = pair.privateKey as RSAPrivateKey;

  String publicKeyString = CryptoUtils.encodeRSAPublicKeyToPem(publicKey);
  String privateKeyString = CryptoUtils.encodeRSAPrivateKeyToPem(privateKey);
  log.info("saving $publicKeyString\nand saving $privateKeyString");

  File publicFile = File(publicFilePath);
  if (!publicFile.existsSync()) {
    publicFile.createSync();
  }
  if (publicKeyString.isNotEmpty) {
    await publicFile.writeAsString(publicKeyString, flush: true);
  }
  File privateFile = File(privateFilePath);
  if (!privateFile.existsSync()) {
    privateFile.createSync();
  }
  if (privateKeyString.isNotEmpty) {
    await privateFile.writeAsString(privateKeyString, flush: true);
  }
}

Future<String> loadFileContentsAsString(String fileName) async {
  ...
  try {
    if (file.existsSync()) {
      ...
      if (contents.isNotEmpty) {
        ...
        log.severe("no contents in pem file $fileName");
      }
    } else {
      log.severe("pem file doesn't exist $fileName");
    }
  } catch (e) {
    log.severe("problem loading pem file $fileName\n $e");
  }
  return "";
}

Future<AsymmetricKeyPair?> retriveKeysFromFile(
    String publicPemFilePath, String privatePemFilePath) async {
  String publicKeyString = await loadFileContentsAsString(publicPemFilePath);
  String privateKeyString = await loadFileContentsAsString(privatePemFilePath);

  //create a instance of RSA key parser
  encrypt.RSAKeyParser keyParser = encrypt.RSAKeyParser();
  ...
  if (privateKeyParser is RSAPrivateKey) {
    privKey = RSAPrivateKey(privateKeyParser.modulus!,
        privateKeyParser.exponent!, privateKeyParser.p, privateKeyParser.q);
    //final plainText = 'hello world';
    return AsymmetricKeyPair(publicKey, privKey);
  } else {
    log.severe("this does not look like the correct private key from pem file");
  }
  return null;
}

Future<String> testPemFiles(String stringToEncrypt, RSAPublicKey publicKey,
    RSAPrivateKey privateKey) async {
  final encrypter = encrypt.Encrypter(
      encrypt.RSA(publicKey: publicKey, privateKey: privateKey));

  final encrypted = encrypter.encrypt(stringToEncrypt);
  final decrypted = encrypter.decrypt(encrypted);
  return decrypted;
}

Future<String> unencryptContentFromPemFiles(String publicPemFilePath,
    String privatePemFilePath, String encryptedContent) async {
  AsymmetricKeyPair? pair =
      await retriveKeysFromFile(publicPemFilePath, privatePemFilePath);
  if (pair != null) {
    final publicKey = pair.publicKey as RSAPublicKey;
    final privateKey = pair.privateKey as RSAPrivateKey;
    final encrypter = encrypt.Encrypter(
    ...
    ///Uint8List.fromList(base64Decode(encryptedContent));
    //utf8.decode(encryptedContent));
    final decrypted =
        encrypter.decrypt(encrypt.Encrypted.fromUtf8(encryptedContent));
    return decrypted;
  }
  return "";
}
...
Future<String> unEncryptStringContentFromBase64(
    String keyAsBase64, String encryptedContent) async {
  final key = encrypt.Key.fromBase64(keyAsBase64);
  final encrypter = encrypt.Encrypter(encrypt.AES(key));
  final iv = encrypt.IV.fromLength(16);
  encrypt.Encrypted encrypted = encrypt.Encrypted.fromBase64(encryptedContent);
  final decryptedString = encrypter.decrypt(encrypted, iv: iv);
  //print("dycrypted??: $decryptedString"); // should be plainText value
  return decryptedString;
}

Future<String> encryptStringContentAsBase64(
    String keyAsBase64, String unEncryptedContent) async {
  //print("handler: encryptStringContentAsBase64() $keyAsBase64");
  final key = encrypt.Key.fromBase64(keyAsBase64);
  final iv = encrypt.IV.fromLength(16);
  final encrypter = encrypt.Encrypter(encrypt.AES(key));
  final encrypted = encrypter.encrypt(unEncryptedContent, iv: iv);
  //final decrypted = encrypter.decrypt(encrypted, iv: iv);
  //print(decrypted); // should be plainText value
  //print("encryptStringContentAsBase64 ${encrypted.base64}");
  return encrypted.base64;
}

Future<bool> encryptAndSaveContent(
    String keyAsBase64, String unEncryptedContent, String filePath) async {
  final key = encrypt.Key.fromBase64(keyAsBase64);
...
Future<String> encryptWithProvidedKey(
...
Future<String> decryptWithProvidedKey(
    String content, String privateKeyString) async {
  //create a instance of RSA key parser
  encrypt.RSAKeyParser keyParser = encrypt.RSAKeyParser();

  //print("handler: decryptWithProvidedKey()");

  //print("parsing key\n$publicKeyString");
  RSAAsymmetricKey privateKeyParser = keyParser.parse(privateKeyString);
  if (privateKeyParser is RSAPrivateKey) {
    final rsaPrivateKey = RSAPrivateKey(privateKeyParser.modulus!,
        privateKeyParser.exponent!, privateKeyParser.p, privateKeyParser.q);
    final encrypter = encrypt.Encrypter(encrypt.RSA(privateKey: rsaPrivateKey));
    var unEncryptedString =
        encrypter.decrypt(encrypt.Encrypted.fromBase64(content));
    //print("unencrypted $unEncryptedString");
    return unEncryptedString;
  }
  return "";
}

Future<bool> encryptAndSaveContentRSA(
    String publicPemFilePath,
    String privatePemFilePath,
    String unEncryptedContent,
    String filePath) async {
  log.info("Saving $filePath .. with content $unEncryptedContent");
  AsymmetricKeyPair? pair =
      await retriveKeysFromFile(publicPemFilePath, privatePemFilePath);
  if (pair != null) {
    final publicKey = pair.publicKey as RSAPublicKey;
...
/***
 *    final encrypted = encrypter.encrypt("hi rob");  1
            Stream<List<int>> inputStream = file.openRead();
          inputStream.listen((chunk) {
            final encrypted = encrypter.encryptBytes(chunk);
...
****/
    ...
    //sink.write(encrypt.Encr);
    Uint8List encryptedBytes = Uint8List(unEncryptedContent.length);
    List<int> elements = [];
    int chunkSize = 50;
    print("${unEncryptedContent.length}");
    for (var i = 0; i < unEncryptedContent.length; i += chunkSize) {
      print("un-encrypting ${unEncryptedContent.substring(i, i + chunkSize)}");
      print("$i");
      var encrypted =
          encrypter.encrypt(unEncryptedContent.substring(i, i + chunkSize));
      elements.addAll(encrypted.bytes);
      if ((unEncryptedContent.length - encryptedBytes.length) < chunkSize) {
        chunkSize = unEncryptedContent.length - encryptedBytes.length;
        if (chunkSize == 0) {
          break;
        }
      }
    }
    await file.writeAsBytes(Uint8List.fromList(elements), flush: true);
    //await file.writeAsBytes(encrypted.bytes, flush: true);
    // Close the IOSink to free system resources.
    //await sink.close();
    log.info("Finished writing to file");
  } else {
    log.severe("Unable to load keys");
  }
  return true;
}

Future<String> loadAndUnencryptContent(
    String keyAsBase64, String encryptedAccountFilePath) async {
  File file = File(encryptedAccountFilePath);
  String encryptedContent = await file.readAsString();
  return unEncryptStringContentFromBase64(keyAsBase64, encryptedContent);
}

Future<String> loadAndUnencryptContentRSA(String publicPemFilePath,
    String privatePemFilePath, String filePath) async {
  ...
  if (pair != null) {
    ...
    if (file.existsSync()) {
      ...
      for (var i = 0; i < bytes.length; i += chunkSize) {
        var more = encrypter.decrypt(encrypt.Encrypted(bytes));
        decryptedString = decryptedString + more;
        chunkSize = bytes.length - decryptedString.length;
        if (chunkSize == 0) {
          break;
        }
      }

      //var decrypted = encrypter.decrypt(encrypt.Encrypted(bytes));
      return decryptedString;
    }
  }
  return "";
